...
%% Code
    ...
    elseif algo == "Region Growth"
        ...
        fe_output.N = length(fe_output.centroids); % The units of the centroids need to be in (mm) make sure of that, else SM-LIS won't work
        fe_output.status = "Done";
        
    elseif algo == "Tagging"
        line_ans = fe_centroiding_2(sis_output, FE_const);
        fe_output.centroids.FE_ID = line_ans(:, 1);
        fe_output.centroids.X = line_ans(:, 2)/1000; % TODO change this
        fe_output.centroids.Y = line_ans(:, 3)/1000;
